...
def get_cbr_for_games(target_games, min_player_count, result_count):
    ...
    for game_a, row in zip(games, cosine_similarity_result):
        result[game_a] = [
            (game_b, value) for game_b, value in zip(games, row) if value >= 0.5
        ]
        result[game_a] = sorted(result[game_a], key=itemgetter(1), reverse=True)[
            1 : result_count + 1
        ]
...
def get_cbr_for_user(
    ...
    for user_game, game_cbr_result in zip(
        user_games_with_score,
        get_cbr_for_games(games_with_score, min_player_count, game_cbr_result_count),
    ):
        if game_cbr_result:
            for sim_game in game_cbr_result:
                if sim_game not in played_games:
                    if sim_game not in result:
                        result[sim_game] = user_game.score * game_cbr_result[sim_game]
                    else:
                        result[sim_game] += user_game.score * game_cbr_result[sim_game]
...
def do_test(user_id, min_player_count, game_cbr_result_count, result_count):
    ...
    quantile = np.quantile([ug.last_played for ug in user_games_with_score], 0.90)
    input_user_games = [
        ug for ug in user_games_with_score if ug.last_played <= quantile
    ]
    input_games = [ug.game for ug in input_user_games]
    check_user_games = [ug for ug in user_games_with_score if ug.last_played > quantile]
    played_games = set()
...
def main():
    ...
    for i in range(100):
        ...
        for user_id in user_ids:
            accuracies.append(
                do_test(user_id, min_player_count, game_cbr_result_count, result_count)
            )
